안녕하세요.
이 저장소는 『데이터 분석 오답 노트』의 실습용 데이터셋을 제공합니다.

책의 내용을 직접 따라하며 실무 중심의 데이터 분석 과정을 학습해보세요.

## 📖 저장소 소개

데이터 분석 실무에서 자주 마주치는 문제와 시행착오를 직접 경험하며,  
문제를 정의하고 데이터를 해석하는 사고 과정을 학습할 수 있도록 구성했습니다.

실습 데이터를 활용하여 책의 예제 코드를 직접 따라해볼 수 있습니다.

---

![데이터 분석 오답 노트](book_cover.png)

### 👩‍💻 **저자**: 주정민, 허현

<p align="center">
  <b>📚 구매 링크</b><br><br>
  <a href="https://www.yes24.com/product/goods/189238126">Yes24</a> ·
  <a href="https://product.kyobobook.co.kr/detail/S000219930257">교보문고</a> ·
  <a href="https://www.aladin.co.kr/shop/wproduct.aspx?ItemId=392629243&srsltid=AfmBOoo9tj8AHgH7_jeXxPlISQuRm4Lk3l4U-kEB1D7XYmtXoJ13LE13">알라딘</a>
</p>


## 💡 실습 전 확인 사항

- 데이터 형식: 모든 실습 데이터는 CSV 형식으로 제공됩니다.

- 인코딩 안내: 한글 깨짐 방지를 위해 UTF-8 형식을 권장합니다. (Excel에서 열 때 주의하세요!)

- 준비 도구: 각 장의 환경설정 안내에 따라 데이터 분석 도구를 미리 준비해 주세요.

---

## 📂 데이터 다운로드 방법

실습 데이터를 내 컴퓨터로 가져오는 방법은 두 가지입니다.

### 1️⃣ ZIP 파일로 다운로드 (추천)
GitHub 계정이 없어도 누구나 간편하게 받을 수 있습니다.
1. 상단의 초록색 **[Code]** 버튼을 클릭합니다.
2. **[Download ZIP]** 을 선택합니다.
3. 내 컴퓨터에서 압축을 해제한 후 실습에 활용합니다.

### 2️⃣ Git Clone (숙련자용)
터미널 또는 CMD에서 아래 명령어를 입력하세요.
```bash
git clone https://github.com/jmin-39/reviewnote.git
```

